...
pub async fn main() -> Result<()> {
    ...
    println!("\nStarting async test...");
    composite(&client, url.clone(), payer.clone(), opts.composite_pid).await?;
...
pub async fn composite<C: Deref<Target = impl Signer> + Clone>(
    ...
    let wrapper_keypair = Arc::new(crate::nonblocking::KeypairSendableWrapper::new(
        payer.clone(),
    ));

    tokio::spawn(async move {
        // Build and send another transaction, using composite account parameters.
        // let payer_from_json = Arc::new(Keypair::from_base58_string(&payer_string));
        let client =
            Client::new_with_options(url.clone(), wrapper_keypair, CommitmentConfig::processed());
